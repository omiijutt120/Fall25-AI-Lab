...
print("--- PHASE 1: Data Exploration (Lab 9) ---")
print('Shape (Rows, Columns):', dt.shape)
print('\nNull Values Count:')
print(dt.isnull().sum())
print('\nColumn Data Types:')
print(dt.dtypes)

print("\n--- PHASE 2: Data Pre-Processing (Lab 10) ---")
...
print(dt.isnull().sum().loc[dt.isnull().sum() > 0]) # Should be empty or only show original non-null columns
...
print('Data cleaned and encoded for modeling.')

# --- PHASE 3: Model Building and Evaluation (Lab 11) ---
print("\n--- PHASE 3: Model Building and Evaluation (Lab 11) ---")

# 1. Splitting Data (70/30)
X_train, X_test, Y_train, Y_test = train_test_split(
    X, Y_e, test_size=0.3, shuffle=True, random_state=42
)
print('X_train shape:', X_train.shape)

# 2. Classifier Training and Scoring
cls = {
...
for name, clf in cls.items():
    ...
    scr.append({
        ...
        'Precision': metrics.precision_score(Y_test, Y_pred, average='weighted', labels=lbl),
        'Recall': metrics.recall_score(Y_test, Y_pred, average='weighted', labels=lbl),
        'F1 Score': metrics.f1_score(Y_test, Y_pred, average='weighted', labels=lbl)
    })
